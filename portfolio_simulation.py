...
def block_bootstrap(sample_data: pd.DataFrame, n: int, block_size: int = 1) -> pd.DataFrame: 
    ...
    for _ in range(n // block_size):
        ...
        start = random.randint(0, len(sample_data.values) - block_size - 1)
        end = start + block_size
            
        # sample block from sample_data and append to new_sample
        new_sample = pd.concat([new_sample, sample_data.iloc[start:end]], ignore_index=True)
...
spx_yearly_returns = spx_yearly_returns.rename(columns={'Date': 'Year', 'Close': 'Returns'})
...
for _ in range(10000):
    sims.append(block_bootstrap(sample_data=spx_yearly_returns, n=40, block_size=10))
