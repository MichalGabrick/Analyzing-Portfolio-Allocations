...
def block_bootstrap(
    data: pd.DataFrame, n: int, block_size: int = 1
) -> pd.DataFrame:
    """
    ...
    new_sample = pd.DataFrame(columns=list(data.columns))

    # sample (n // block_size) blocks of data points
    for _ in range(n // block_size):
        # randomly select start and end of a block of data points (used to sample from data)
        start = randint(0, len(data.values) - block_size - 1)
        end = start + block_size

        # sample block from data and append to new_sample
        new_sample = pd.concat(
            [new_sample, data.iloc[start:end]], ignore_index=True
        )
...
def portfolio_sim(
    ...
    rebalance: bool,
    rebalancing_band: int = 0,
) -> pd.DataFrame:
    """
    Simulate the future values of a portfolio of securities.

        Parameters:
                stock_returns (pd.DataFrame): Dataframe of annual historical returns of stocks.
                bond_returns (pd.DataFrame): Dataframe of annual historical returns of bonds.
                stock_allocation (int): Percentage of assets in portfolio allocated to stocks representated as a decimal from 0 to 1.
    ...
    for x in range(1, len(stock_returns)): # TODO: MAYBE NEEDS A '+ 1'
        # asset value for 'year n' is (value for 'year n-1' + annual contribution) * asset returns for that year
        stock_value = (
            future_values.iloc[x - 1]["Stock Value"] + (annual_contribution * stock_allocation)
        ) * (1 + stock_returns.loc[x]["Returns"])

        bond_value = (
            future_values.iloc[x - 1]["Bond Value"] + (annual_contribution * (1 - stock_allocation))
        ) * (1 + bond_returns.loc[x]["Returns"])
